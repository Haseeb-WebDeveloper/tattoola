  ...
  return (
    <View className="mb-4 p-6 rounded-2xl border-2 border-gray bg-black/40">
      <View className="flex-row items-center justify-between mb-4">
        <View>
          <View className="w-32 h-4 bg-gray/30 rounded mb-2" />
          <View className="w-48 h-3 bg-gray/20 rounded" />
        </View>
        <View className="items-end">
          <View className="w-16 h-6 bg-gray/30 rounded mb-2" />
          <View className="w-20 h-3 bg-gray/20 rounded" />
        </View>
      </View>
      <View className="">
        <View className="w-56 h-3 bg-gray/20 rounded mb-2" />
        <View className="w-40 h-3 bg-gray/20 rounded mb-2" />
        <View className="w-48 h-3 bg-gray/20 rounded mb-2" />
        <View className="w-28 h-3 bg-gray/20 rounded mb-2" />
      </View>
    </View>
  );
}
...
export default function ArtistStep13V2() {
  ...
  // No full-screen loading; we show inline skeletons instead

  return (
    <View
      className="flex-1 bg-black pb-40 relative"
    >
      {/* Header */}
      <AuthStepHeader />
      ...
      <View className="items-center   mb-4 mt-8">
        <View className="flex-row items-center gap-1">
          {Array.from({ length: totalStepsDisplay }).map((_, idx) => (
            <View
              key={idx}
              className={`${idx < 13 ? (idx === 12 ? "bg-foreground w-4 h-4" : "bg-success w-2 h-2") : "bg-gray w-2 h-2"} rounded-full`}
            />
          ))}
        </View>
      </View>
      ...
      <View className="px-6 mb-6">
        <View className="flex-row items-center justify-center bg-black/40 rounded-full p-1">
          <TouchableOpacity
            ...
            className={`flex-1 py-2 px-4 rounded-full border  ${step13.billingCycle === "MONTHLY" ? "bg-primary border-transparent" : "bg-transparent border-foreground"}`}
          >
            <Text
              className={`text-center font-neueBold ${step13.billingCycle === "MONTHLY" ? "text-foreground" : "text-foreground/60"}`}
            >
              Monthly
            </Text>
          </TouchableOpacity>
          <TouchableOpacity
            ...
            className={`flex-1 py-2 px-4 rounded-full border  ${step13.billingCycle === "YEARLY" ? "bg-primary border-transparent" : "bg-transparent border-foreground"}`}
          >
            <Text
              className={`text-center font-neueBold ${step13.billingCycle === "YEARLY" ? "text-foreground" : "text-foreground/60"}`}
            >
              Yearly
            </Text>
          </TouchableOpacity>
      ...
      <View className="flex-row justify-between px-6 py-4 bg-background absolute bottom-0 left-0 right-0 z-10">
        ...
        <TouchableOpacity
          ...
          className={`rounded-full px-8 py-4 ${loading || submitting || !canProceed ? "bg-gray/40" : "bg-primary"}`}
        >
