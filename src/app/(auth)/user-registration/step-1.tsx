...
import type { FormErrors, UserRegistrationStep1 } from '@/types/auth';
...
export default function UserRegistrationStep1() {
  const { 
    step1, 
    updateStep, 
    setErrors, 
  ...
  const [formData, setFormData] = useState<UserRegistrationStep1>({
    firstName: '',
    lastName: '',
    phone: '',
  });
  ...
  useEffect(() => {
    if (step1 && Object.keys(step1).length > 0) {
      setFormData(step1 as UserRegistrationStep1);
    }
  }, [step1]);

  const handleInputChange = (field: keyof UserRegistrationStep1, value: string) => {
